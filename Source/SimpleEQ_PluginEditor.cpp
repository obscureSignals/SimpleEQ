                                       ...
                                       const float rotaryStartAngle, const float rotaryEndAngle, juce::Slider& slider)
{
    ...
    // If control is bypassed, make the the thumb color dim
    if (slider.isEnabled()){
...
void RotarySliderWithLabels::paint(juce::Graphics &g)
{
    ...
    for( int i = 0; i < numChoices; ++i )
    {
        ...
        auto ang = jmap(pos, 0.f, float(numChoices)-1, startAng, endAng);
...
{
    // Updates the filters owned by the the editor with the current control values
