...
struct FFTDataGenerator
{
    ...
    FFTDataGenerator(const float negativeInfinity) : negativeInfinity(negativeInfinity)
    {
    ...
    void produceFFTDataForRendering(const juce::AudioBuffer<float>& audioData, float newFFTOrder)
    {
        ...
        for (int aveIdx = 0; aveIdx < numAves; aveIdx++ ){
            for (int dataIdx = 0; dataIdx < fftSize; dataIdx++){
                fftDataAve[dataIdx] = fftDataAve[dataIdx] + fftDataVector[aveIdx][dataIdx]/(float)numAves;
            }
        }
    ...
    int getFFTSize() const { return 1 << fftOrder; } // FFT size is 2^fftOrder
...
struct AnalyzerPathGenerator
{
    ...
    AnalyzerPathGenerator(const float negativeInfinity) : negativeInfinity(negativeInfinity)
    {
...
//    binWidth is width of FFT bins in Hz = sampleRate/fftSize
                      ...
                      float binWidth)
    {
        ...
        // There are two becuse gin spline wants doubles but path wants floats - im sure it could be done with one, but this was easier
        juce::Array<juce::Point<int>> points;
        juce::Array<gin::Point<double>> dpoints;
        pSpline.preallocateSpace(3 * (int)fftBounds.getWidth());
        
        // create horizontal map from dB value to pixel space
        const auto negInf = negativeInfinity;
        auto map = [bottom, top, negInf](float v)
        {
            return juce::jmap(v,negInf, 0.f,
                              float(bottom+6), top);
        };
        
        float y; // Height of bin
        float prevY; // Height of previous bin
        ...
        int prevBinX; // Previous bin's horizontal coordinate
        ...
        {
            y = map(renderData[binNum]); // Set horizontal coordinate for current value
            ...
            if( !std::isnan(y) && !std::isinf(y) ) // There should not be NaNs or infs
            {
                ...
                int binX = std::floor(normalizedBinX * width); // Horizontal coordinate in pixels
                if (binX != prevBinX) { // If the horizontal coordinate is not the same as it was for the last bin
                    if (runFlag or lastFlag) { // If run flag or the last flag are tripped, this must be the end of a run run
                        prevY = minRunY;
                    ...
                    if (!firstFlag) { // Add the previous points to our point arrays
                        points.add ({ prevBinX, int (prevY)});
                        dpoints.add ({ double (prevBinX), double (prevY)});
                    }
                }
                else { // If the horizontal coordinate is same as it was for the last bin
                    if (!runFlag) { // If the run flag was not already tripped, this is a new run
                        minRunY = juce::jmin(prevY,y); // Find min of this y and the the last y - smaller values are higher up in gui
                        runFlag = true; // We are running
                    }
                    else { // If the run flag was already tripped, we are already in a run
        ...
        for (int x = points.getFirst().getX(); x < points.getLast().getX(); x++)
        {
            ...
            pSpline.lineTo (float (x), float (y));
        }
        
        // Enclose the path so we cna fill it
        pSpline.lineTo (points.getLast().getX(), bottom+10);
        pSpline.lineTo (points.getFirst().getX(), bottom+10);
        pSpline.lineTo (points.getFirst().toFloat());

        // Puhs path to path Fifo
...
struct PathProducer
{
    PathProducer(StereoSummingSampleFifo<PlayBackEQAudioProcessor::BlockType>& sssf, PlayBackEQAudioProcessor& p, const float negativeInfinity) :
    Fifo(&sssf), audioProcessor(p), FFTDataGenerator(negativeInfinity), pathGenerator(negativeInfinity), negativeInfinity(negativeInfinity)
    {
...
{
public:
    SpectrumDisplay(PlayBackEQAudioProcessor&);
    ~SpectrumDisplay();
    void paint(juce::Graphics& g) override;
...
juce::Timer
{
    ResponseCurveComponent(PlayBackEQAudioProcessor&);
